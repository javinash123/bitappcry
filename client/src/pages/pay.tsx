...
import visaLogo from "@assets/visa_1773099402245.png";
import mastercardLogo from "@assets/mastercard_1773099402244.png";
import amexLogo from "@assets/amexx_1773099402243.png";
...
export default function Pay() {
  ...
  return (
    <div className="min-h-screen bg-background font-sans flex items-center justify-center p-4">
      <main className="w-full max-w-2xl space-y-3">
        {/* Company Header */}
        <Card className="border-2 border-border/50">
          <CardContent className="pt-3 pb-3">
            <div className="space-y-0.5">
              <h2 className="text-sm sm:text-base font-bold font-heading">{companyName}</h2>
              <p className="text-[10px] text-muted-foreground leading-none">Invoice #{invoiceId}</p>
              <p className="text-xs font-semibold mt-1">{totalToPay.toFixed(2)} AED (Total with fee {totalToPay.toFixed(2)} AED)</p>
        ...
        <div className="grid grid-cols-2 gap-2">
          <Button variant="outline" className="h-8 py-0 flex items-center justify-center border-2 border-border/50 hover:bg-accent/50">
            <span className="text-blue-600 font-bold italic text-xs">PayPal</span>
          </Button>
          <Button variant="outline" className="h-8 py-0 flex items-center justify-center border-2 border-border/50 hover:bg-accent/50">
        ...
        <Card className="border-2 border-border/50">
          <CardHeader className="py-1.5 px-4">
            <CardTitle className="text-[10px] uppercase tracking-wide">
              Amount to Pay (AED)
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-1.5 pb-2.5 px-4">
            <div className="flex gap-2">
              <Input
                ...
                className="flex-1 border-2 border-border/50 focus:border-primary/50 h-8 text-xs"
                data-testid="input-amount-to-pay"
              />
              <Button
                onClick={handleSelectMax}
                className="bg-primary hover:bg-primary/90 h-8 px-2 text-[10px] font-semibold"
        ...
        <Card className="border-2 border-border/50">
          <CardHeader className="py-1.5 px-4">
            <CardTitle className="text-[10px] uppercase tracking-wide">Select Cryptocurrency</CardTitle>
          </CardHeader>
          <CardContent className="space-y-1.5 pb-2.5 px-4">
            <RadioGroup value={selectedCrypto} onValueChange={setSelectedCrypto} className="space-y-1">
              {cryptocurrencies.map((crypto) => (
                <label
                  key={crypto.id}
                  htmlFor={crypto.id}
                  className={`flex items-center gap-2 p-2 rounded-lg transition-all duration-300 cursor-pointer border-2 ${
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-semibold text-[11px] leading-tight">{crypto.name}</div>
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <img src={mastercardLogo} alt="Mastercard" className="h-3 w-auto object-contain" />
                    <img src={amexLogo} alt="Amex" className="h-3 w-auto object-contain" />
                  </div>
                </label>
              ))}
            </RadioGroup>
        ...
        <Button
          onClick={handlePay}
          className="w-full bg-black hover:bg-black/90 text-white h-9 text-[11px] font-semibold"
        ...
        <Link href={`/invoice/${invoiceId}`}>
          <Button
            variant="outline"
            className="w-full border-2 border-primary text-primary hover:bg-primary/5 h-9 text-[11px] font-semibold"
